    ...
    if (!isCorruptedPacket && (ackNum >= expectedAck)) {
        ...
        for (int i = 0; i < numPacketsToSend; i++) {
            ...
            if (!sentLastPacket) {
                ...
                cout << "printing window after building new pkt" << endl;
                printWindow(window);
...
void sendCurrentWindow(char window[][BUFFER_SIZE], int windowHead,
    ...
    for (int i = 0; i < WINDOW_SIZE; i++) {
        while (!sendPacket(socketfd, receiverAddr, receiverAddrLength, window[(windowHead + i) % WINDOW_SIZE]))
            continue;
        printPacketInfo("DATA", getSeqNum(window[(windowHead + i) % WINDOW_SIZE]));
    ...
    while (1) {
        ...
        if (receiverLength > 0) {
            ...
            while (1) {
                ...
                if (ackMsgLength <= 0) {
                    if (TIMEOUT_FLAG) {
                        sendCurrentWindow(window, windowHead, socketfd, &receiverAddr, receiverAddrLength);
                    }
                    continue;
                } 
